...
// inline void get_input_array_shapes(const , size_t *rows,
//                                    size_t *features) {
//                                     buffer.
...
class Index {
  std::shared_ptr<glass::Builder> inner = nullptr;

public:
  Index(const Index& rhs): inner(rhs.inner) {}
  Index(
    int dim,
  ...
  //   size_t features,
  // ) {
  //   index->Build(data.data(), rows);
  //   return Graph(index->GetGraph());
  // }
};

EMSCRIPTEN_BINDINGS(Glass) {
  using namespace emscripten;
  class_<Index>("Index")
    .class_function("new", &Index::constructor);
    // .property("inner", &Index::inner);
}
